...
public class Eje5 {
   ...
   public static void main(String[] args) {
      ...
      Curso cus1 = new Curso(true, "Curso de HTML, CSS y JS", LocalDate.parse("2025-01-19"),
            LocalDate.parse("2024-09-01"), 001, "Jose Andres", "Desarrollo Web", 219.99, List.of(c3, c4));
      Curso cus2 = new Curso(false, "Curso de Java", LocalDate.parse("2025-07-01"), LocalDate.parse("2025-01-21"), 002,
            "Pedro Alcala", "Programacion", 259.99, List.of(c1, c2));

      PlataformaEducativa pe = new PlataformaEducativa(List.of(cus1, cus2),
            List.of(e1, e2, e3), "EVGD");

      JsonSerializer<LocalDate> serializer = (src, typeOfSrc, context) -> new JsonPrimitive(src.format(formato));
      ...
      double suma = 0;
      double promedio = 0;
      int totalclasificaicones = 0;

      for (Curso cursos : pe.getCursos()) {

         for (Calificacion c : cursos.getCalificaciones()) {
            suma += c.getNota();
            totalclasificaicones++;

         }

         promedio = suma / totalclasificaicones;
